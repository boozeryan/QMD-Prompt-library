  ...
  <script>
    class PromptLibraryApp {
      ...
      _initFirebase() {
        ...
        catch (e) { console.error("Firebase 初始化失敗:", e); alert("雲端資料庫初始化失敗！請檢查您的 Firebase 設定。"); }
      }
      ...
      async _seedDataToFirestore() {
        ...
        try { await batch.commit(); this._showToast("已成功將預設提示詞庫上傳至雲端！"); } 
        catch (e) { console.error("植入初始資料失敗:", e); }
      }
      ...
      _renderCategoryChips() {
          ...
          allChip.className = 'chip clickable'; allChip.textContent = '全部分類'; allChip.dataset.category = 'all';
      ...
      async _savePrompt() {
          ...
          if (!form.checkValidity()) { form.reportValidity(); this._showToast("儲存失敗：請檢查所有必填欄位！", "error"); return; }
      ...
      async _importDataToFirestore(prompts, categories) {
        ...
        prompts.forEach(p => {
            const promptRef = this.db.collection("prompts").doc();
            batch.set(promptRef, {
                task: p.task || "無標題", category: p.category || "未分類", prompt: p.prompt || "",
